...
namespace HWAssignment06.Controllers
{
    public partial class ProductsController : Controller
    {
        ...
        {
            var objecttodel = db.Products.FirstOrDefault(x => x.ProductId == id);
            db.Products.Remove(objecttodel);
            db.SaveChanges();
            productsViewModel.Remove(productsViewModel.First(x => x.id == id));
            return Json(productsViewModel, JsonRequestBehavior.AllowGet); // <<<<<<<<< You will need to change this to a Json return.
        }
